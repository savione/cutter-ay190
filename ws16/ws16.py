...
def hlle(hyd):
    ...
    # right are qm[:,i+1]
    for i in range(1,hyd.n-2):
        fluxl[0,i] = hyd.rhom[i] * hyd.velp[i]
        fluxl[1,i] = hyd.rhop[i] * hyd.velp[i]**2 + hyd.pressp[i]
        fluxl[2,i] = (hyd.rhop[i] * hyd.eps[i] + 0.5 * hyd.rhop[i]
                      * hyd.velp[i]**2 + hyd.pressp[i]) * hyd.velp[i]

        fluxr[0,i] = hyd.rhom[i] * hyd.velm[i]
        fluxr[1,i] = hyd.rhom[i] * hyd.velm[i]**2 + hyd.pressm[i]
        fluxr[2,i] = (hyd.rhom[i] * hyd.eps[i] + 0.5 * hyd.rhom[i]
                      * hyd.velm[i]**2 + hyd.pressm[i]) * hyd.velm[i]

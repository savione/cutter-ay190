...
def hlle(hyd):
    ...
    fluxl[0,:] = hyd.rhom.copy() * hyd.velp.copy()
    fluxl[1,:] = hyd.rhop.copy() * hyd.velp.copy()**2 + hyd.pressp.copy()
    fluxl[2,:] = (hyd.rhop.copy() * hyd.eps.copy() + 0.5 * hyd.rhop.copy()
                  * hyd.velp.copy()**2 + hyd.pressp.copy()) * hyd.velp.copy()

    fluxr[0,:] = hyd.rhom.copy() * hyd.velm.copy()
    fluxr[1,:] = hyd.rhom.copy() * hyd.velm.copy()**2 + hyd.pressm.copy()
    fluxr[2,:] = (hyd.rhom.copy() * hyd.eps.copy() + 0.5 * hyd.rhom.copy()
                  * hyd.velm.copy()**2 + hyd.pressm.copy()) * hyd.velm.copy()
...
for reconstruction_type in ["pc", "mc", "minmod"]:

    print(reconstruction_type)
    data[reconstruction_type] = {}
    # initialize
    hyd = mydata(nzones)
    ...
    while(t < tend):
        ...
        if figs:
            ...
            if(i % 10 == 0):
                ...
                mpl.clf()
                mpl.plot(hyd.x,hyd.rho,"r-", lw=8)
                timestring = "time: %5.3f" % (t)
                ax = mpl.gca()
                mpl.text(0.7,0.88,timestring,fontsize=25,
                        horizontalalignment="left",rotation="horizontal",
                        transform=ax.transAxes)
                mpl.draw()

        if t > 0.2:
            data[reconstruction_type]["x"] = hyd.x
            data[reconstruction_type]["rho"] = hyd.rho
            break
...
pc_plot, = mpl.plot(data["pc"]["x"], data["pc"]["rho"], lw=8)
mc_plot, = mpl.plot(data["mc"]["x"], data["mc"]["rho"], '--', lw=8)
minmod_plot, = mpl.plot(data["minmod"]["x"], data["minmod"]["rho"], '--', lw=8)

mpl.xlabel("Position $x$", labelpad=10)
mpl.ylabel(r"Density $\rho$", labelpad=10)
mpl.legend( (pc_plot, mc_plot, minmod_plot), ("Piecewise-Constant", "TVD-MC2", "TVD-minmod"),
            frameon=False, loc="lower left" )

ax = mpl.gca()
mpl.text(0.7,0.88,"time: 2.0",fontsize=25,
        horizontalalignment="left",rotation="horizontal",
        transform=ax.transAxes)

mpl.savefig("comparison.pdf")
mpl.show()
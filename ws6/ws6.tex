...
I worked with my Partner David Vartanyan on this worksheet. It took about 14 astronomical units to complete (2 hours).

\subsection{The Discrete Fourier Transform}

\noindent (a)

I implemented the matrix formulation of the discrete Fourier transform in \texttt{dft.py}. The relative errors between the output \(y(x)\) of my DFT and the output \(z(x)\) of \texttt{numpy.fft.fft} for a vector \(x\) can be considered to be
\[
\text{relative error} = \frac{|y - z|}{|z|}.
\]
In general \(y\) and \(z\) are complex vectors. The relative error between my DFT and \texttt{numpy}'s FFT for a random vector of ten dimensions was \(1.67 \ee{-15}\). \\

\noindent (b-c)
\begin{figure}[H]
\begin{centering}
 \hspace{-1cm} \includegraphics[width=0.8\textwidth]{fig-b+c.pdf}
 \caption{The runtime of two discrete Fourier transform implementations as a function of input vector size. Over the depicted range, the \texttt{numpy} FFT appears to execute in constant time, whereas the matrix DFT increases approximately one log unit of runtime for one-half log unit of input vector size, indicating that runtime \(\sim N^2\). This plot demonstrates that FFT is \emph{fast}.}
 \label{fig-b+c}
\end{centering}
\end{figure} 

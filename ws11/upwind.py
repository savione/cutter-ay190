...
def calc_rhs(yold):
    if v > 0:
        # upstream
        y = yold[1:] - v * (dt / dx) * (yold[1:] - yold[:-1])
        y = np.append(0, y) # this is a dummy value which will get wiped out by
                        # application of the boundary condition
    elif v < 0:
        # downstream
        y = yold[:-1] - v * (dt / dx) * (yold[1:] - yold[:-1])
        y = np.append(y, 0) # again, a dummy value
    else:
        y = yold
    return y
...
mpl.ylabel(r"Error $\varepsilon$", labelpad=15)
mpl.xlabel("Iteration number", labelpad=10)
